...
- Status do pedido (RF-02):
  ...
  - Após pagamento (apenas dono do restaurante):
    - `PUT /orders/{id}/status` — livre entre `PREPARANDO`, `SAIU_PARA_ENTREGA`, `ENTREGADOR_NO_LOCAL`, `ENTREGUE` (exige `payment_status=PAGO`)
  - `POST /orders/{id}/cancel`:
    - Cliente: só em `EM_CADASTRAMENTO` ou `AGUARDANDO_PAGAMENTO`
    - Dono: nos status posteriores (`PREPARANDO` em diante)
